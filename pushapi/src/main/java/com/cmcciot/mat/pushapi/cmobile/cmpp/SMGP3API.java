 ...
 * </p>
 * <p>
 * Copyright: All rights reserved by www.simpleteam.com. Copyright (c) 2000 ~
 * 2009
 * </p>
 * <p>
...
public class SMGP3API {
	 ...
	 *         </p>
	 */
	public SMGP3API(String enterpriseCode, String serviceCode, String account,
			String password) {
		// Create sequence.
	...
	public synchronized SMGPPacket readPacket() throws IOException {
		try {
			// Return result.
			return connection != null ? (SMGPPacket) connection.readPacket()
					: null;
		} catch (BufferException e) {
			throw new IOException(e.getMessage());
		} catch (ConnectionException e) {
	...
	private boolean login() {
		...
		if (!login.isValid()) {
			...
			if (LogRequests.isRequested(EventID.SESSION | EventID.EXCEPTION))
				SimpleLog.log(CLASS_NAME, "login").message(
						"invalid SMGP3 login !").end();
			return false;
		}
		...
		try {
			...
			if (smgp == null) {
				// Log event.
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "login").message(
							"fail to read packet !").end();
				return false;
			}
			// Check command
			if (smgp.command != SMGPRequestID.LOGIN_RESPONSE) {
				...
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "login").message(
							"invalid login response packet !").end();
				return false;
			}
			// Check sequence.
			if (smgp.sequence != login.sequence) {
				...
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "login").message(
							"sequence is not according to previous !").end();
			}
			// Get login response.
			...
			if (response.status != SMGPStatus.SUCCESS) {
				...
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "login").begin().append(
							SMGPStatus.toString(response.status)).append(" !")
							.end();
				return false;
			}
			// Set authenticated.
	...
	public boolean connect(String remoteHost, int remotePort) {
		try {
			...
			// Log event.
			if (LogRequests
					.isRequested(EventID.SMGP_PACKET | EventID.EXCEPTION))
	...
	private boolean logout() {
		...
		try {
			...
			if (smgp == null) {
				...
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "logout").message(
							"fail to read packet !").end();
				return false;
			}
			// Check command.
			if (smgp.command == SMGPRequestID.EXIT) {
				...
				// Log event.
				if (LogRequests.isRequested(EventID.SESSION
						| EventID.INFORMATION))
					SimpleLog.log(CLASS_NAME, "logout").message(
							"exit request was received !").end();
			} else if (smgp.command == SMGPRequestID.EXIT_RESPONSE) {
				// Log event.
				if (LogRequests.isRequested(EventID.SESSION
						| EventID.INFORMATION))
					SimpleLog.log(CLASS_NAME, "logout").message(
							"exit response was received !").end();
			} else {
				// Log event.
				if (LogRequests
						.isRequested(EventID.SESSION | EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "logout").message(
							"invalid exit response packet !").end();
				return false;
			}
			// Set authenticated.
			authenticated = false;
	...
	public void close() {
		...
		if (!connection.isClosed()) {
			...
			try {
				...
				// Log event.
				if (LogRequests.isRequested(EventID.SMGP_PACKET
						| EventID.EXCEPTION))
					SimpleLog.log(CLASS_NAME, "close").exception(e, false)
							.end();
			}
			// Log event.
			if (LogRequests.isRequested(EventID.SMGP_PACKET
					| EventID.INFORMATION))
				SimpleLog.log(CLASS_NAME, "close").message(
						"connection was closed !").end();
		} else {
			// Log event.
			if (LogRequests.isRequested(EventID.SMGP_PACKET
					| EventID.INFORMATION))
				SimpleLog.log(CLASS_NAME, "close").message(
						"connection has already been closed !").end();
		}
	}
	 ...
	 * 
	 * <p>
	 * It is better to sperate submit and deliver processing, although this
	 * example give a mixed processing.
	 * </p>
	 * 
	 * @param args
	 *            Arguments.
	...
	public static void main(String[] args) {
		...
		if (!logDirectory.exists() && !logDirectory.mkdir()) {
			System.out
					.println("SMGP3API.main : fail to create log directory !");
			return;
		}
		// Log mode.
		...
		if (args.length > 1) {
			...
			// Print.
			System.out.println("SMGP3API.main : log mode(0x"
					+ Hex.toString(logMode) + ") !");
		}
		// Initialize log.
		...
		try {
			// Create API.
			SMGP3API smgp3Api = new SMGP3API(enterpriseCode, serviceCode,
					account, password);
			// Connect.
			if (smgp3Api.connect("127.0.0.1", 7890)) {
				// Print.
				...
				while (reportCount < totalCount) {
					...
					if (sendCount < totalCount) {
						// Create submit.
						SMGP3Submit submit = new SMGP3Submit(smgp3Api
								.nextSequence());
						...
						// Message Content
						submit.msg_content = "Hello World !"
								.getBytes("US-ASCII");
						// Message Length
					...
					if (smgp3Api.available()) {
						...
						if (packet instanceof SMGP3Deliver) {
							...
							SMGPDeliverResponse response = new SMGPDeliverResponse(
									packet.sequence);
							// Set status.
							response.status = 0;
							// Set response.
							...
							// Write response.
							smgp3Api.writePacket(response);// Do response as
							// soon as possible.
							...
							if ((deliver.is_report & 0x0f) == 1) {
								// Print.
								System.out
										.println("SMGP3API.main : report/status("
												+ deliver.stat + ")");
							} else {
								// Print.
								System.out
										.println("SMGP3API.main : normal/source("
												+ deliver.src_term_id + ")");
							}
							//
							// /////////////////////////////////////////////////////////////////////////
						...
						// Check instance.
							...
							SMGPActiveTestResponse response = new SMGPActiveTestResponse(
									packet.sequence);
							// Write response.
						...
						// Check command.
							...
							// Print.
							System.out.println("SMGP3API.main : submit/result("
									+ response.msg_id + ")");
