...
{
    private static Logger logger = LoggerFactory.getLogger(AccessLbsInterFaceTools.class);
    ...
    public static String getGPSLocation(Integer type, String celltowers)
    {
        ...
        gpsLoc = request(url + requestUrl + params.toString(), requestUrl
                + params.toString(), SICode, secureKey, false);
    ...
    public static String getGPSLocationByGaode(Map<String, Object> map)
    {
        ...
        try
        {
            ...
            urlParamSb.append("?")
                    ...
                    .append("&nearbts="
                            + URLEncoder.encode(nearbtsBuffer.toString()))
                    .append("&serverip=" + IP)
                    .append("&output=json")
                    .append("&key=" + gaodeKey);
            jsonStr = request(url + requestUrl + urlParamSb.toString(),
                    requestUrl + urlParamSb.toString(),
                    SICode,
                    secureKey);
            return jsonStr;
        }
        ...
        {
            logger.error("调用高德gps接口错误", e);
            throw new LogicException(ClientErrorNumber.ERROR_INTERFACE,
                    "调用高德地图 接口异常(~call gaode MAP interface failed!)");
        }
        
    }
    
    private static String request(String fullUrl, String requestUrl,
            String sicode, String key, boolean logger)
    {
        try
        {
    ...
    private static String sign(String operation, String requestUrl,
            ...
            String sicode, String key)
    {
        try
        {
            ...
            reader.close();
            connection.disconnect();
            logger.info("位置能力平台返回：请求消息为"+ fullUrl +";返回码为:"+code +";返回消息为:"+ resp.toString());
            return resp.toString();
        }
        catch (Exception e)
        {
            logger.error("调用位置能力接口报错！",e);
            return "";
        }
    }
